...
The Bronzecoin Core daemon can be configured to act as a trusted "border
router", implementing the bronzecoin wire protocol and relay, making
...
The ZeroMQ feature in Bronzecoin Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building bronzecoind:
...
For instance:
    ...
    $ bronzecoind -zmqpubhashtx=tcp://127.0.0.1:28332 \
               -zmqpubhashtx=tcp://192.168.1.2:28332 \
               -zmqpubrawtx=ipc:///tmp/bronzecoind.tx.raw \
...
These options can also be provided in bronzecoin.conf.
...
From the perspective of bronzecoind, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into bronzecoind directly. Furthermore, no information is
...
using. Bronzecoind appends an up-counting sequence number to each
