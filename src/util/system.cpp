...
const char * const BITCOIN_CONF_FILENAME = "bronzecoin.conf";
...
#else
    const char* pszModule = "bronzecoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Bronzecoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/Bronzecoin";
#else
    // Unix-like
    return pathRet / ".bronzecoin";
#endif
#endif
}
